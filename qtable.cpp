...
QTableLine QTable::findRow( QTableLine values, int fromRow, int fromColumn )
{
    if( !p_table ) return QTableLine();
    return QTableLine();
}

QVariant QTable::findCell( int cellColumn, QTableLine values, int fromRow, int fromColumn )
{
    if( !p_table ) return QVariant();
    return QVariant();
}
