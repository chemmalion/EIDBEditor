...
// TODO add a check if data.length() > contents.length().  Then try to store the original and revert the compression method to be uncompressed
    ...
    if (header.file_name.size() > 0xffff) {
        qWarning("QtZip: Filename too long, chopping it to 65535 characters");
...
/*!
    Just unzip zip archive to target directory.
    Returns true if zip was successfully unpacked, otherwise returns false.
*/
bool QtZipReader::unzip( const QString & zipFileName, const QString & targetDir )
{
    QString tDir = targetDir;
    tDir = QDir::fromNativeSeparators( tDir );
    if( !tDir.endsWith('/') ) tDir += '/';
    QtZipReader zip_reader( zipFileName );
    bool result = false;
    if( zip_reader.exists() && zip_reader.isReadable() )
    {
        result = zip_reader.extractAll( tDir );
    }
    zip_reader.close();
    return result;
}
...
QByteArray QtZipReader::fileData(const QString &fileName) const
{
    ...
    qWarning() << "QtZip: Unknown compression method";
    return QByteArray();
}
...
{
    QString dest = QDir::fromNativeSeparators( destinationDir );
    while( dest.endsWith('/') ) dest.remove( dest.size()-1, 1 );

    QDir baseDir(dest);

    // create directories first
    QList<FileInfo> allFiles = fileInfoList();
    foreach (QtZipReader::FileInfo fi, allFiles)
    {
        const QString absPath = dest + QLatin1Char('/') + fi.filePath;
        if (fi.isDir) {
        ...
        else if( fi.isFile || fi.isSymLink )
        {
            ...
            //qDebug() << "making: " << qfi.absolutePath();
            if( !baseDir.mkpath( qfi.absolutePath() ) )
                return false;
            //qDebug() << "done";
            if (!QFile::setPermissions(qfi.absolutePath(), fi.permissions))
                return false;
        }
    }
...
/*!
    Just pack zip archive from source directory.
    Returns true if zip was successfully packed, otherwise returns false.
*/
bool QtZipWriter::zip( const QString & zipFileName, const QString & sourceDirPath )
{
    QtZipWriter zip( zipFileName );
    if (zip.status() != QtZipWriter::NoError)
        return false;

    zip.setCompressionPolicy(QtZipWriter::AutoCompress);

    QString path = sourceDirPath;
...
{
    d->addEntry(QtZipWriterPrivate::File, fileName, data);
}
...
void QtZipWriter::addFile(const QString &fileName, QIODevice *device)
{
    ...
    d->addEntry(QtZipWriterPrivate::File, fileName, device->readAll());
    if (opened)
        device->close();
}
...
{
    d->addEntry(QtZipWriterPrivate::Symlink, fileName, QFile::encodeName(destination));
}
